...
module AdaBelief
    function back_propagation(t::Int64, α::Float64, batch_size::Int64, Current_Layer::Any, Last_Layer::Any, Next_Layer::Any, β1::Float64, β2::Float64, ϵ::Float64)
        ...
        Current_Layer.propagation_units = try
            Current_Layer.get_PU(Current_Layer.weights, ∇biases)
        catch e
            Current_Layer.get_PU(∇biases)
        end
        ...
        if Current_Layer.update_weights
            ...
            Current_Layer.Sdw = Current_Layer.Sdw .* β2 + (weights_belief .* weights_belief) .* (1-β2)
            Threads.@threads for i in eachindex(Current_Layer.weights)
                Current_Layer.weights[i] -= α*(Current_Layer.Vdw[i]/(1-β1^t))/(sqrt(Current_Layer.Sdw[i]/(1-β2^t))+ϵ)*Current_Layer.weights_prop[i]
            end
        end
        ...
        if Current_Layer.update_biases
            ...
            Threads.@threads for i in eachindex(Current_Layer.biases)
                Current_Layer.biases[i] -= α*(Current_Layer.Vdb[i]/(1-β1^t))/(sqrt(Current_Layer.Sdb[i]/(1-β2^t))+ϵ)*Current_Layer.biases_prop[i]
            end
        end
    end

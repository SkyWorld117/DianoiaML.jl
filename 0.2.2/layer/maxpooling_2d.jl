...
module maxpooling_2d
    mutable struct MaxPooling_2D
        ...
        function MaxPooling_2D(;input_size::Int64, input2D_size::Tuple{Int64, Int64}, kernel_size::Tuple{Int64, Int64}, step_x::Int64=kernel_size[2], step_y::Int64=kernel_size[1], activation_function::Module, reload::Bool=false)
            if reload
            ...
            new(save_MaxPooling2D, load_MaxPooling2D, activate_MaxPooling2D, PU_MaxPooling2D, false, false, input_size, conv_num_per_row*conv_num_per_col, activation_function, input2D_size, kernel_size, step_x, step_y)
        end
    end
    ...
    function create_weights(layer::MaxPooling_2D, input::Array{Float32}, b)
        ...
        for i in 0:layer.layer_size-1
            index = layer.step_x*(i%conv_num_per_row) + layer.input2D_size[2]*layer.step_y*(i÷conv_num_per_row)
            max_value = -Inf
            register = 0
            for j in 1:layer.kernel_size[1]
                for k in 1:layer.kernel_size[2]
                    if input[index+k,b]>max_value
            ...
            layer.weights[i+1,register,b] = Int8(1)
        end
    end

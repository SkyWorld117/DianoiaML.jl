    ...
    if output:
        for e in range(epochs):
            ...
            if epigenetics:
                ...
                pools[0].layers[-1].propagation_units = Vectorize(loss_function.propagation, matrix_a=pools[0].layers[-2].output, matrix_b=output_data)
                error = abs(np.sum(pools[0].layers[-1].propagation_units))
                del pools[0].layers[-1]
            else:
                error = min_error
            best = 0
            for i in range(1, population):
                ...
                pools[i].layers[-1].propagation_units = Vectorize(loss_function.propagation, matrix_a=pools[i].layers[-2].output, matrix_b=output_data)

                if error>abs(np.sum(pools[i].layers[-1].propagation_units)):
                    error = abs(np.sum(pools[i].layers[-1].propagation_units))
                    best = i

                del pools[i].layers[-1]

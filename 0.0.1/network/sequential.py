...
from loss_function.quadratic_loss import Vectorized_Quadratic_Loss_Propagation_Units
    ...
    def train(self, input_data = [], output_data = [], loss_function = 'Quadratic_Loss', optimizer = 'Gradient_Descent', learning_rate = 0.01, epochs = 6000):
        ...
        for e in range(epochs):
            ...
            for i in range(len(self.layers)-2, 0, -1):
                ...
                self.layers[i].propagation_units, self.layers[i].weights, self.layers[i].bias = Gradient_Descent(learning_rate, self.layers[i], self.layers[i-1], self.layers[i+1])

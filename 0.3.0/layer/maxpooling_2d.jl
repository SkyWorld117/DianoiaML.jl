...
module maxpooling_2d
    ...
    function PU_MaxPooling2D(layer::MaxPooling_2D, ∇biases::Array{Float32})
        batch_size = size(∇biases, 2)
        propagation_units = zeros(Float32, (layer.input_size, batch_size))
        if layer.step_x>=layer.kernel_size[2] && layer.step_y>=layer.kernel_size[1]
            for b in 1:batch_size
                #for (x, y) in enumerate(layer.weights[:,b])
                #    propagation_units[y,b] = ∇biases[x,b]
                #end
                Threads.@threads for x in 1:size(layer.weights, 1)
                    propagation_units[layer.weights[x,b],b] = ∇biases[x,b]
                end
            end
        else
            Threads.@threads for i in 1:layer.input_size
                for b in 1:batch_size
                    for x in findall(y->y==i, layer.weights[:,b])
                        propagation_units[i,b] += ∇biases[x,b]
                    end
                end
            end
        end
    ...
    function create_value(layer::MaxPooling_2D, input::Array{Float32}, b::Int64, i::Int64, conv_num_per_row::Int64, conv_num_per_col::Int64)

module TConv2DM
    ...
    mutable struct TConv2D
        ...
        function TConv2D(;input_shape::Tuple, filter::Int64, padding::Int64=0, kernel_size::Tuple{Int64, Int64}, strides::Tuple{Int64, Int64}=(1,1), activation_function::Module, randomization::Bool=true)
            conv_num_per_col = (input_shape[1]-1)*strides[1]+2*padding-kernel_size[1]+2
            conv_num_per_row = (input_shape[2]-1)*strides[2]+2*padding-kernel_size[2]+2
            ...
            new(save_TConv2D, load_TConv2D, activate_TConv2D, init_TConv2D, update_TConv2D, input_shape, output_shape, kernel_size, activation_function, filters, biases, padding, filter, strides, ∇weights, Vdw, Sdw, ∇biases_c, Vdb, Sdb)
        end
    end

    function init_TConv2D(layer::TConv2D, mini_batch::Int64)
        layer.padding_input = zeros(Float32, ((layer.input_shape[1]-1)*layer.strides[1]+2*layer.padding+1, (layer.input_shape[2]-1)*layer.strides[2]+2*layer.padding+1, layer.input_shape[3], mini_batch))
        ...
        layer.pre_δ = zeros(Float32, ((layer.input_shape[1]-1)*layer.strides[1]+2*layer.padding+1, (layer.input_shape[2]-1)*layer.strides[2]+2*layer.padding+1, layer.input_shape[3], mini_batch))
        layer.δ = zeros(Float32, layer.input_shape..., mini_batch)
    end

    function activate_TConv2D(layer::TConv2D, input::Array{Float32})
        x, y = layer.strides
        @avxt for i in axes(input, 1), j in axes(input, 2), c in axes(input, 3), b in axes(input, 4)
            layer.padding_input[layer.padding+(i-1)*x+1, layer.padding+(j-1)*y+1, c, b] = input[i, j, c, b]
        end
        ...
        @avxt for i in axes(layer.value, 1), j in axes(layer.value, 2), f in axes(layer.value, 3), b in axes(layer.value, 4)
            ...
            for k₁ in axes(layer.filters, 3), k₂ in axes(layer.filters, 4), c in axes(layer.filters, 2)
                s += layer.padding_input[i-1+k₁, j-1+k₂, c, b] * layer.filters[f, c, k₁, k₂]
            end
    ...
    function update_TConv2D(layer::TConv2D, optimizer::String, input::Array{Float32}, δₗ₊₁::Array{Float32}, α::Float64, parameters::Tuple, direction::Int64=1)
        ...
        @avxt for i in axes(layer.∇biases, 1), j in axes(layer.∇biases, 2), f in axes(layer.∇biases, 3), b in axes(layer.∇biases, 4)
            for k₁ in axes(layer.filters, 3), k₂ in axes(layer.filters, 4), c in axes(layer.filters, 2)
                layer.pre_δ[i-1+k₁, j-1+k₂, c, b] += layer.filters[f, c, k₁, k₂]*layer.∇biases[i, j, f, b]
            end
        end

        x, y = layer.strides
        @avxt for i in axes(layer.δ, 1), j in axes(layer.δ, 2), c in axes(layer.δ, 3), b in axes(layer.δ, 4)
            layer.δ[i, j, c, b] = layer.pre_δ[layer.padding+(i-1)*x+1, layer.padding+(j-1)*y+1, c, b]
        end

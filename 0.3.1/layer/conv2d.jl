module conv2d
    ...
    function activate_Conv2D(layer::Conv2D, input::Array{Float32})
        ...
        =#
        layer.value = zeros(Float32, (size(layer.weights, 1), size(input, 2)))
        kernel_size = (size(layer.filters,3), size(layer.filters,4))
        conv_num_per_row = (layer.input2D_size[2]-kernel_size[2])÷layer.step_x+1
        conv_num_per_col = (layer.input2D_size[1]-kernel_size[1])÷layer.step_y+1
        Conv2D_mul!(layer.value, layer.weights, input, layer.filters, layer.step_x, layer.step_y, layer.input2D_size, kernel_size, layer.unit_size, conv_num_per_row, conv_num_per_col) #layer.value = layer.weights*input
    ...
    function PU_Conv2D(layer::Conv2D, ∇biases::Array{Float32})
        layer.propagation_units = zeros(Float32, (size(layer.weights, 2), size(∇biases, 2)))

        kernel_size = (size(layer.filters,3), size(layer.filters,4))
        conv_num_per_row = (layer.input2D_size[2]-kernel_size[2])÷layer.step_x+1
        conv_num_per_col = (layer.input2D_size[1]-kernel_size[1])÷layer.step_y+1
        Conv2D_PU!(layer.propagation_units, layer.weights, ∇biases, layer.filters, layer.step_x, layer.step_y, layer.input2D_size, kernel_size, layer.unit_size, conv_num_per_row, conv_num_per_col)

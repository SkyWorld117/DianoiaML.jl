module SGD
    ...
    function update_weights!(weights, ∇biases, output, α, weights_prop)
        @avx for x in axes(∇biases, 1), y in axes(output, 1)
            c = 0.0f0
            for b in axes(∇biases, 2)
                c += ∇biases[x,b]*output[y,b]
            end
            weights[x,y] -= c*α*weights_prop[x,y]
        end
    end
    ...
    function back_propagation(α::Float64, Current_Layer::Any, Last_Layer::Any, Next_Layer::Any)
        ...
        if Current_Layer.update_weights
            update_weights!(Current_Layer.weights, ∇biases, Last_Layer.output, α, Current_Layer.weights_prop) # Current_Layer.weights -= ∇biases*transpose(Last_Layer.output).*α.*Current_Layer.weights_prop
        end
    ...
    function fit(;sequential::Any, input_data::Array{Float32}, output_data::Array{Float32}, loss_function::Any, monitor::Any, α::Float64=0.01, epochs::Int64=20, batch::Real=32)
        ...
        for e in 1:epochs
            ...
            monitor.func(sequential.layers[end-1].output, batch_output_data)
        end
    end
end
#81.790142

...
from loss_function.quadratic_loss import Quadratic_Loss_Propagation_Units, Quadratic_Loss
    ...
    def train(self, input_data = [], output_data = [], loss_function = 'Quadratic_Loss', optimizer = 'Gradient_Descent', learning_rate = 0.01, epochs = 6000):
        ...
        for e in range(epochs):
            ...
            if loss_function == 'Quadratic_Loss':
                self.layers[len(self.layers)-1].propagation_units = Vectorize(Quadratic_Loss_Propagation_Units, 'Loss_Function_Propagation_Units', matrix_a=self.layers[len(self.layers)-2].output, matrix_b=output_data)
                print('current loss =',np.sum(Vectorize(Quadratic_Loss, 'Loss_Function', matrix_a=self.layers[len(self.layers)-2].output, matrix_b=output_data)))

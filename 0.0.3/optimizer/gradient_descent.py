...
def Gradient_Descent(Q, id, start_position, learning_rate, batch_size, Current_Layer, Last_Layer, Next_Layer):
...
def Multiprocessing_Gradient_Descent(learning_rate, batch_size, Current_Layer, Last_Layer, Next_Layer):
    ...
    for i in range(threads):
        if b>0:
            processes.append(multiprocessing.Process(target=Gradient_Descent, args=(Q, i, start_position, learning_rate, a+1, Current_Layer, Last_Layer, Next_Layer)))
            b = b-1
            start_position = start_position+a+1
        else:
            processes.append(multiprocessing.Process(target=Gradient_Descent, args=(Q, i, start_position, learning_rate, a, Current_Layer, Last_Layer, Next_Layer)))

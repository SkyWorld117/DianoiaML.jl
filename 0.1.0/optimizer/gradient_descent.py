...
def Gradient_Descent(sequential=None, input_data=[], output_data=[], loss_function=None, learning_rate=0.01, epochs=6000, output=True):
    ...
    if output:
        for e in range(epochs):
            ...
            sequential.layers[len(sequential.layers)-1].propagation_units = Vectorize(loss_function.propagation, matrix_a=sequential.layers[len(sequential.layers)-2].output, matrix_b=output_data)

            print('  Current loss =',Vectorize(Standard_Loss.function, matrix_a=sequential.layers[len(sequential.layers)-2].output, matrix_b=output_data))
            #print('  Current accuracy = ', 1-Vectorize(Loss_Function, matrix_a=sequential.layers[len(sequential.layers)-2].output, matrix_b=output_data)/batch_size)
    ...
    del sequential.layers[len(sequential.layers)-1]
    gc.collect()
